...
int main(void){
    // ORDERS
    Order order1 = create_order("Rebecca Raspberry", 111, 2019, 3, 30, 6.10);
    Order order2 = create_order("Fiona Framboise", 116, 2019, 3, 29, 17.0);
    Order order3 = create_order("Neal Naranja", 225, 2020, 1, 6, 18.7);
    Order order4 = create_order("Hannah Hindbaer", 120, 2019, 3, 29, 12.0);
    ...
    double to_add = 5.60;
    printf("\tadding $%.2f to order1\n", to_add);
    add_to_charge(&order1, to_add);
    print_order(&order1);
    
    printf("\nPart 2 - Exercise 2: Test update_date:\n");
    int new_year = 2011, new_month = 4, new_day = 19;
    printf("\tSetting order2 date to %d/%d/%d\n", new_year, new_month, new_day);
    update_date(&order2, new_year, new_month, new_day);
    print_order(&order2);
    ...
    */
    ...
    printf("\tadding a $%.2f to all order in the array \n", to_add);
    add_surcharge(my_orders, 4, to_add);
    print_orders(my_orders, 4);
...
void print_order(Order* o_ptr) {
   printf("\t\tOrder #%d by %s on %d/%d/%d. Price: $%.2f\n", 
           o_ptr->order_number, 
           o_ptr->customer_name, 
           o_ptr->delivery_date.year, 
           o_ptr->delivery_date.month, 
           o_ptr->delivery_date.day, 
           o_ptr->charge);
}
