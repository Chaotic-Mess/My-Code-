...
(async () => {
  ...
  const deepScan = async (url, depth = 0, visited = new Set()) => {
    ...
    try {
      ...
      d.querySelectorAll("a[href], source[src], iframe[src], embed[src]").forEach(n => {
        ...
        else if (/viewContent|content\/|quickLink.*fileId=/i.test(u))
          deepScan(u, depth + 1, visited).then(sub => sub.forEach(x => found.add(x)));
      });
      return [...found];
    } catch (e) {
  ...
  ui.querySelector("#bs-start").onclick = async () => {
    ...
    const add = async (mods, pre="") => {
      for (const m of mods || []) {
        ...
        for (const t of m.Topics || []) {
          ...
          let targets = [u];
          if (doDeep && (/\.html?/i.test(u) || /viewContent|quickLink|content\/\d+/i.test(u))) {
    ...
    const date = new Date(), tag = `(${String(date.getFullYear()).slice(2)}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')})`;
